 ...
 */
int* multiply(int** matrix, int* vector, int n, int m){
    int* c = new int[n];
    //malloc(sizeof(int)*m);
...
int* multiplyCacheBlocking(int** matrix, int* vector, int n, int m, int tile_size){
    assert(m % tile_size == 0);
    int* c = new int[n];
    for(unsigned int i =0; i < m; i++){
        c[i] = 0;
    }
    for(unsigned int jj = 0; jj < m; jj+=tile_size){
        for(unsigned int i  = 0; i < n; i++){
            for(unsigned j = jj; j < jj + tile_size; j++){
                c[i] += matrix[i][j]*vector[j];
                cout << "c[" << i << "]: = " << "M[" << i << "][" << j << "]*b[" << j << "]" << endl;
                //cout << c[i] << " = " << matrix[i][j] << " * " << vector[j] << endl;
            }
...
void printElements(int** matrix, int* vector, int *c, int n, int m){
    cout << "Matrix: " << endl;
    for(unsigned int i = 0; i < n; i++){
        for(unsigned int j = 0; j < m; j++){
            cout << matrix[i][j] << "\t";
...
int main(int argc, char* argv[]){
    int m = 2000, n = 3000, use_cache = 0, tile_size=4;
    if (argc > 4){
        n = atoi(argv[1]);
        m = atoi(argv[2]);
